...
import { getToken } from './services/login/components/helpers/TokenHelpers'
...
export const getData = async (datatype, superId, id) => {
    if (!datatype || !id) 
...
export const createData = async (datatype, superId, data) => {
    if (!datatype || !data) 
...
export const updateData = async (datatype, superId, id, data) => {
    if (!datatype || !id || !data) 
...
export const deleteData = async (datatype, superId, id) => {
    if (!datatype || !id) 
        throw new Error('Datatype and id are required')
